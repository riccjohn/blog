...
pubDate: '2026-01-21'
heroImage: '../../assets/images/blog_posts/claude-ux-designer-pt-2/andras-vas-Bd7gNnWJBkU-unsplash.jpg'
---

import { Image } from 'astro:assets'
import RepoLink from '../../components/RepoLink.astro'
import pluginInstall from '../../assets/images/blog_posts/claude-ux-designer-pt-2/install_plugin.png'
import homepageDesktopMockup from '../../assets/images/blog_posts/claude-ux-designer-pt-2/home-desktop-2026-01-15.png'
...
A `SKILL.md` (located at `.claude/skills/screenshot/SKILL.md`) file that describes what the skill does and how to use it. Notice how the frontmatter at the top specifies the name, description, model, and allowed tools. This frontmatter is added to the main context window, but the rest of the skill is only loaded when the skill is invoked.
...
With a Claude skill, I can specify which model to use for that skill. I don't need the screenshot skill using the same model (and number of tokens) as the designer skill. The designer skill benefits from a larger context window and more advanced reasoning capabilities, while the screenshot skill is more procedural and just runs a script to do the bulk of its work. By specifying a smaller model for the screenshot skill, I can save on token usage and costs.

In the [frontmatter of a skill](https://code.claude.com/docs/en/skills#frontmatter-reference), you can specify a bunch of options, including the model to use when that skill is invoked:

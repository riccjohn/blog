...
- [How To Start Learning AI](/blog/how-to-start-learning-ai.mdx)
- [LangChain Confusion](/blog/langchain-confusion.mdx)
...
My first method of testing was asking it questions manually. Asking it questions manually didn't work for a lot of reasons, but mainly: It's time-consuming and error prone, it's not targeting retrieval only, and I don't get a quantitative gauge of the effect my changes are having.
...
Not only that, but having most test questions pass except for one and fixing it only to watch a different question break—was incredibly frustrating. Solve one problem, create another, and have no idea why.

All of this combined just resulted in slow feedback loops and tech debt disguised as progress.
...
To actually get that <mark>recall@k</mark> value, we need to create some fixtures that have pre-written questions, the raw file I expect the answer to be in, and a substring for a portion of the answer. I then create a small script to loop over each fixture and check whether the expected substring appears in the retrieved chunks.
...
The fixture I built is a json file with 12 objects containing a sample question, the substring that the answer is found in, and the source file. I tried to cover a range of questions that had specific answers so I could be confident that retrieval was working across the knowledge base. This includes answers that are found in formats like markdown tables rather than only being in paragraphs of text.
